...
void execute(const ProcessorStateStruct& current, ProcessorStateStruct& next) {
    // we clear the executing list to rebuild it from current so we can handle exception flushes
    next.ExecutingInstructions.clear();

    // Process each instruction currently in the ALUs (reading from current, not next)
    for (int i = 0; i < (int)current.ExecutingInstructions.size(); i++) {
        
        // work on a copy of the current instruction
        ExecutingInstructionALU ex = current.ExecutingInstructions[i]; 
        
        ex.CyclesLeft--; // decrease the cycles left
        ...
        if (ex.CyclesLeft == 0) {
            ...
            switch (ex.OpCode) {
                case Opcode::ADD: 
                case Opcode::ADDI:
                    ex.Result = ex.OpAValue + ex.OpBValue;
                    break;
                case Opcode::SUB:
                    ex.Result = ex.OpAValue - ex.OpBValue;
                    break;
                ...
                case Opcode::DIVU:
                    if (ex.OpBValue == 0) {
                        ex.ExceptionCaused = true;
                        ex.Result = 0; // won't be committed
                    } else {
                        ex.Result = ex.OpAValue / ex.OpBValue;
                    }
                    break;
            ...
            for (int j = 0; j < (int)next.ActiveList.size(); j++) {
                if (next.ActiveList[j].PC == ex.PC &&
                    ...
                    next.ActiveList[j].Exception = ex.ExceptionCaused; // set if it caused exception
                    break;
                }
            }

            // we write result and clear busy bit if it did not cause exception 
            if (!ex.ExceptionCaused) {
                next.PhysicalRegisterFile[ex.rd] = ex.Result;
                next.BusyBitTable[ex.rd] = false;
                ...
                for (int j = 0; j < (int)next.IntegerQueue.size(); j++) {
                    ...
                    if (!next.IntegerQueue[j].OpAIsReady &&
                        next.IntegerQueue[j].OpARegTag == ex.rd) {
                        next.IntegerQueue[j].OpAIsReady = true;
                        next.IntegerQueue[j].OpARegTag = 0;  // clear tag when ready
                        next.IntegerQueue[j].OpAValue = ex.Result;
                    }
                    // if their opB is not ready and wait for this register
                    if (!next.IntegerQueue[j].OpBIsReady &&
                        next.IntegerQueue[j].OpBRegTag == ex.rd) {
                        next.IntegerQueue[j].OpBIsReady = true;
                        next.IntegerQueue[j].OpBRegTag = 0;  // clear tag when ready
                        next.IntegerQueue[j].OpBValue = ex.Result;
                    }
                }
            }
        } else {
            // if the instruction is not finished yet we keep it for the next cycle
            // but only if commit didn't trigger an exception that flushes the pipeline
            if (!next.ExceptionFlag) {
                next.ExecutingInstructions.push_back(ex);
            }
        }
    }
}
